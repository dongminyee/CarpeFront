...
async function fetchAndUpdateImages(year) {
    ...
    // 화면에 보이는 글자 바꾸기
    triggerText.textContent = year+"기";

    // 기존 선택된 스타일 지우고, 방금 클릭한 애한테 스타일 주기
    options.forEach(opt => {
        opt.classList.remove('selected');
        if(opt.textContent==(year+"기")) opt.classList.add('selected');
    });
    
    try {
        //1. API로 데이터 요청
        const response = await fetch(API_URL+"/generation?gen="+year);
        if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', ()=>{
    mainTxt.textContent = "21기";
    fetchAndUpdateImages("21");
    initButtons();
...
const initButtons = ()=>{
    ...
    for(let i=22;i>=17;i--){
        ...
        if(i==21) newLi.className = "custom-option selected";
